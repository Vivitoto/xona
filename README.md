...
- **批量元数据**：扫描目录后为多个视频生成自包含元数据，使用紧凑汇总、折叠详情和批量级预览/执行，适合 100+ 文件场景。
...
1. 在 **本地元数据生成 → 批量整理** 中选择来源目录并扫描视频。
2. 选择要处理的视频，填写批量元数据规则、封面规则和批量输出规则。
3. 点击 **生成批量元数据**，Xona 会为所有选中视频生成自包含元数据，并只展示紧凑列表与汇总。
4. 点击 **生成全部预览**，统一生成 NFO、封面和整理计划；失败、处理中、可执行条目会优先显示，详细日志默认折叠。
5. 确认无误后点击 **执行全部可执行计划**。

批量整理面向大文件数优先设计，不再把每个条目载入单个编辑器逐条保存；需要精细手动修改单个文件时，切回 **单个整理** 工作流处理该视频。
